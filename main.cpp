...
void updateBoard(char (&b)[b_h][b_w],int col, char disc) {
...
int checkBoard(char b[b_h][b_w], char disc) {
	...
	for (int rowNo = b_h - 1; rowNo >= 0; rowNo--) {
		...
		for (int colNo = 0; colNo < b_w; colNo++) {
			(b[rowNo][colNo] == disc) ? count++: count = 0;				// count++ if match, reset counter if not match
	...
	for (int rowNo = 0; rowNo < b_h; rowNo++) {
		...
		while (x >= 0 && x < b_w && y >= 0 && y < b_h) {		// make sure it doesnt add out of bound
			(b[rowNo + y][x] == disc) ? count++ : count = 0;
			if (count == 4) { return 0; }
			x++;
			y++;
		}
	...
	for (int colNo = 0; colNo < b_w; colNo++) {
		...
		int y{ 0 };
		while (x >= 0 && x < b_w && y >= 0 && y < b_h) {		// make sure it doesnt add out of bound
			(b[y][colNo + x] == disc) ? count++ : count = 0;
			if (count == 4) { return 0; }
			x++;
			y++;
		}
...
int main(){
	...
	human p2 = human(2, 'X', yellow, "Marco");
	...
	while (true) {
		...
		if (move % 2 == 0) {
			curr_player = &p1;
			updateBoard(board, p1.askMove(board), p1.playerChess);
		}
		else {
			curr_player = &p2;
			updateBoard(board, p2.askMove(board), p2.playerChess);
		}
		move++;
